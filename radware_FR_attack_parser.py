...
# ---------------- Global variable to change merge window ---------------- #
# You can change this variable and re-run without passing --gap-min.
MERGE_WINDOW_MIN = 5  # default merge window in minutes
...
# Column aliases to be resilient to minor header differences.
COL_ALIASES = {
...
def resolve_columns(df: pd.DataFrame) -> dict:
    ...
    if missing:
        raise ValueError(
            f"Missing required column(s) in CSV: {missing}. Available columns: {list(df.columns)}"
        )
    return resolved
...
def group_campaigns_by_dst(
    ...
    """Group events into attack campaigns keyed by Destination IP (and optionally Destination Port)
    by merging overlapping windows or windows within gap_minutes."""

    w = df.copy()

    # Destination Port normalized for union label or key
    ...
    for c in campaigns:
        ...
        threat_cats = sorted(set(edf.get(cols.get("Threat Category", "Threat Category"), pd.Series(dtype="object")).dropna().astype(str)))
        attack_names = sorted(set(edf.get(cols.get("Attack Name", "Attack Name"), pd.Series(dtype="object")).dropna().astype(str)))

        # Ports union (numeric only)
        ...
        rows.append({
            ...
            **({"Destination Port (key)": int(c["PortKey"]) if (split_by_port and pd.notna(c["PortKey"])) else np.nan} if split_by_port else {}),
            ...
            "Threat Categories": ", ".join(threat_cats) if threat_cats else "N/A",
            "Vectors (Attack Names)": "; ".join(attack_names) if attack_names else "N/A",
...
def main():
    ap = argparse.ArgumentParser(
        description="Summarize Radware attacks grouped by Destination IP and time windows (optional port split)."
    )
    ap.add_argument("input_csv", help="Path to Radware CSV export")
    ap.add_argument("--out-csv", default="Attack_Campaigns_By_DstIP_Time.csv",
                    help="Output CSV path (default: %(default)s)")
    ap.add_argument("--out-xlsx", default=None,
                    help="Also write Excel (.xlsx) to this path (optional)")
    ap.add_argument("--gap-min", type=int, default=MERGE_WINDOW_MIN,
                    help="Max gap in minutes to merge events into same attack window (default: MERGE_WINDOW_MIN=%(default)s)")
    ap.add_argument("--time-format", default="%m.%d.%Y %H:%M:%S",
                    help="Datetime format for Start/End Time (default: %%m.%%d.%%Y %%H:%%M:%%S). Leave empty to infer.")
    ap.add_argument("--split-by-port", action="store_true",
                    help="Add Destination Port to the grouping key (split attacks per dest port).")
    ap.add_argument("--encoding", default=None,
                    help="Optional file encoding override (e.g., latin1). If not set, tries utf-8 then latin1.")
    ap.add_argument("--skip-bad-lines", action="store_true",
                    help="Skip malformed CSV lines if any (on_bad_lines='skip').")
    ...
    try:
        ...
        print(f"[ERROR] Failed reading CSV: {ex}", file=sys.stderr)
        sys.exit(1)
    ...
    # Build campaigns using CLI gap (overrides global MERGE_WINDOW_MIN)
    out = group_campaigns_by_dst(
        df=df,
        cols=cols,
        gap_minutes=int(args.gap_min),
        split_by_port=bool(args.split_by_port),
    )
